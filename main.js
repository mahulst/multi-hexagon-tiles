...
let drawCoords = true;
let shape = [
  { hex: { row: 3, col: 7 }, parts: allHexParts },
  { hex: { row: 4, col: 7 }, parts: allHexParts },
...
function redraw() {
  ...
  function hexCenter(q, r) {
    ...
    const centerY =
      hexSize * (Math.sqrt(3) * r + (Math.sqrt(3) / 2) * (q % 2)) + padding;
        ...
        self.findIndex((t) => {
          ...
          return (
            (JSON.stringify(a) === JSON.stringify(c) &&
              JSON.stringify(b) === JSON.stringify(d)) ||
  ...
  function drawFilledHexShape(shape, color) {
  ...
  function drawHexGrid(rows, cols) {
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        const x = isEven
          ? hexWidth * 1.5 * col
          : hexWidth * 1.5 * col + (hexWidth * 1.5) / 2;
  ...
  drawHexGrid(8, 16);
  drawFilledHexShape(shape, "rgb(48,76,122)");
  drawFilledHexShape(
    [
      { hex: { row: 3, col: 11 }, parts: allHexParts },
      { hex: { row: 3, col: 12 }, parts: allHexParts },
      { hex: { row: 2, col: 13 }, parts: allHexParts },
    ],
    "rgb(224,98,54)",
  );
  drawFilledHexShape(
    [
      { hex: { row: 2, col: 1 }, parts: allHexParts },
      { hex: { row: 3, col: 2 }, parts: allHexParts },
      { hex: { row: 3, col: 3 }, parts: allHexParts },
    ],
    "rgb(215,166,75)",
  );
}
...
function rotateHexShapeClockwise(hexShape) {
  ...
  return hexShape.map((shape, index) => {
    if (index === 0) {
      ...
      const neighbour = findHexNeighbor(
        centerHex,
